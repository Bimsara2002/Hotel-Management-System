...
.offer-card {
  background: linear-gradient(135deg, #f7a4a4ff, #ffffff);
  border-left: 6px solid #fd2222ff;
  padding: 20px;
  border-radius: 12px;
  margin-bottom: 15px;
  transition: 0.3s;
...
if (isset($_POST['searchRoom'])) {
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      echo '
      <div class="room-card">
        <img src="'.$row['image'].'" alt="'.$row['type'].'">
        <h3>'.$row['type'].' Room</h3>
    ...
    echo "<p style='text-align:center;color:#004080;'>No rooms found matching your search.</p>";
  }
} else {
  // Show first 6 available rooms
  $sql = "SELECT * FROM room WHERE status='Available' LIMIT 6";
